...
//        String substr = filePath.length() > format ? filePath.substring(0, format) : filePath;
    ...
    private void addSheet(CellRangeAddress cellRangeAddress, Sheet sheet, int rowNumber, int cellNumber, String textCell){
        Row row;Cell cell;
    ...
    public void createFile(String dirName, String textField) throws SQLException, ClassNotFoundException, IOException, NoSuchAlgorithmException {
        ...
        this.addSheet(new CellRangeAddress(0,1,0,3), sheet, 0, 0, titleFile);
        cellStyle.setAlignment(HorizontalAlignment.CENTER);
        cell.setCellStyle(cellStyle);

        this.addSheet(new CellRangeAddress(2,2,0,3), sheet, 2, 0, textField);
        cellStyle.setAlignment(HorizontalAlignment.CENTER);
        cell.setCellStyle(cellStyle);

        this.addSheet(new CellRangeAddress(j+2,j+2,0,3), sheet, j+2, 0, "Руководитель");
        RegionUtil.setBorderBottom(BorderStyle.THIN, new CellRangeAddress(j+2,j+2,0,3), sheet);

        this.addSheet(new CellRangeAddress(j+4,j+4,0,3), sheet, j+4, 0, "Глав. бухгалтер");
        RegionUtil.setBorderBottom(BorderStyle.THIN, new CellRangeAddress(j+4,j+4,0,3), sheet);

        this.addSheet(new CellRangeAddress(j+6,j+6,0,3), sheet, j+6, 0, "Контрольная сумма");
        cellStyle.setAlignment(HorizontalAlignment.CENTER);
        cell.setCellStyle(cellStyle);

        this.addSheet(new CellRangeAddress(j+7,j+7,0,3), sheet, j+7, 0, this.md5(filename));
        cellStyle.setAlignment(HorizontalAlignment.CENTER);
        cell.setCellStyle(cellStyle);
        ...
        writeBATFile.println("gpg -r "+key+" -e "+format+counterFile+"_UPD."+ format+".CSV");
        writeBATFile.close();
